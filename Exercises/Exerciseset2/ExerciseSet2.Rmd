...
* We need 5 factors to explain atleast 50% of the variance in this data
* 0.93 , 0,87 , 0,86
* First 10 components can be described as Futures of Bonds ,Currncey, European markets, American markets, European exchange rates, Oil and gas , Metals, Grains, Precious metals, Chinese stocks
* Answer to Question number 4, By changing the dates in line 167 we can also obtain data for precrisis times(Not done because procedure is the same and only requires copay pasting of the code)
```{r}
PreCrisis_data = futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) <"2007-10-01",]
```
```{r}
show_data = round(cor(PreCrisis_data),2)
iprint.df(show_data, scale=TRUE)
```
```{r}
...
```
5
The charts above show that in the pre-crisis bull market, 15 principal components had an eigenvalue >1 which accounted for 69.95% of cumulative explained variance. In contrast, in the financial crisis years only 13 principal components have an eigenvalue >1 which accounts for 75.4% of cumulative explained variance. There is also a significant difference between correlations of the comodities or 64 indiceshelp
*
*
*
*
...
* MAfreq sets the windwo from 1-250 with distance by 20 this should be the length of averaging so chaning it will change the output significantly and make the moving average even smoother(if the window is increased). after normalizing the program is iterating data to find the average from the window created and save it into a matrix. this will create an extra sequence of line which will be according to the data or returns provided. 

* As said earlier this changes the output so that the daily returns are effected. 
*
*
*
*
